<template>
    ...
    <div class="mb-2 max-w-lg mx-auto">
      <div class="relative">
        <input ref="searchInput" :value="query" @input="search(($event.target as HTMLInputElement).value)"
          ...
          class="w-full pl-10 pr-10 py-3 bg-teal-900/90 backdrop-blur-xl rounded-xl border border-teal-700/50 outline-none placeholder:text-(--ui-text-dimmed) text-(--ui-text) transition-all duration-200 focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/20" />
        <UIcon name="i-lucide-search" class="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-(--ui-text-dimmed)" />
        <button v-if="query" @click="clearSearch" class="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 text-(--ui-text-dimmed) active:text-(--ui-text-muted)">
      ...
      <Transition enter-active-class="transition ease-out duration-150" enter-from-class="opacity-0 translate-y-2" enter-to-class="opacity-100 translate-y-0"
        ...
        <div v-if="searchFocused && (query || searching)" class="absolute bottom-full left-0 right-0 mb-2 max-h-72 overflow-y-auto overscroll-contain rounded-xl bg-teal-900 border border-teal-700/50 shadow-2xl">
          ...
          <div v-else-if="results.length" class="py-1">
            <template v-for="item in results" :key="item.id">
              <NuxtLink v-if="item.type === 'task'" :to="`/tasks/${item.id}`" @click="clearSearch"
                class="flex items-center gap-2.5 px-4 py-3 transition-colors active:bg-teal-800/70">
          ...
          <div v-else-if="query" class="p-4 text-center text-sm text-(--ui-text-dimmed)">No results found</div>
        </div>
      </Transition>
    </div>
    ...
    <nav class="bg-teal-900/80 backdrop-blur-xl rounded-2xl border border-teal-700/50 shadow-lg">
      <div class="flex items-center justify-around h-16 max-w-lg mx-auto px-1">
        <NuxtLink v-for="item in items" :key="item.to" :to="item.to"
          class="flex flex-col items-center justify-center flex-1 h-14 rounded-xl text-[10px] font-medium tracking-wide transition-all duration-200 active:scale-95"
  ...
  <Transition enter-active-class="transition ease-out duration-150" enter-from-class="opacity-0" enter-to-class="opacity-100"
    ...
    <div v-if="searchFocused" class="fixed inset-0 z-40 bg-black/40" @click="clearSearch" />
  </Transition>
</template>
